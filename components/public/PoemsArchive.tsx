...
interface PoemsArchiveProps {
  initialPoems: Poem[];
}

const ITEMS_PER_PAGE = 9;

export function PoemsArchive({ initialPoems }: PoemsArchiveProps) {
  ...
  const availableYears = useMemo(() => {
    ...
    initialPoems.forEach((poem) => {
      if (poem.publicationDate) {
        const year = new Date(poem.publicationDate).getFullYear().toString();
        years.add(year);
      }
    });
  ...
  // Filter poems based on search query and selected year
...
const matchesYear = selectedYear === "All" || poemYear === selectedYear;
  ...
  }, [initialPoems, searchQuery, selectedYear]);
    ...
    <div className="space-y-16">
      ...
      <div className="flex flex-col md:flex-row gap-6 justify-between items-center border-b border-border pb-8">
        ...
        <div className="relative w-full md:w-1/2 lg:w-1/3">
          ...
          <input
            type="text"
            placeholder="Search poems by title or word..."
            value={searchQuery}
            onChange={(e) => {
      ...
      {filteredPoems.length > 0 ? (
        ...
        <div className="py-24 text-center border border-border bg-muted/10">
          <p className="font-serif text-2xl text-primary">No poems found.</p>
